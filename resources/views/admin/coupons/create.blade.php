                    ...
                    <form method="POST" action="{{ route('admin.coupons.store') }}">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" placeholder="e.g., SUMMER20" value="{{ old('name') }}" required>
                                @error('name')
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" class="form-control @error('discount') is-invalid @enderror" id="discount" name="discount" placeholder="e.g., 10" value="{{ old('discount') }}" required min="0">
                                @error('discount')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="datetime-local" class="form-control @error('valid_until') is-invalid @enderror" id="valid_until" name="valid_until" required
                                   min="{{ $carbon::now()->addDay()->format('Y-m-d\TH:i') }}"
                                   value="{{ old('valid_until', $carbon::now()->addDay()->format('Y-m-d\TH:i')) }}" >
                            @error('valid_until')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
